...
class AppMeasurement {
    ...
    /**
     * Singleton interface to obtain a single AppMeasurement
     * instance within the entire request.  Do not invoke this method
     * if multiple AppMeasurement instances are being used within
     * the request.
     *
     * @return Singleton AppMeasurement instance
     */
    public static function getInstance()
    {
        if (self::$singletonInstance == NULL) {
            self::$singletonInstance = new AppMeasurement();
        }
    ...
    /**
     * Track the current state of the AppMeasurement instance
     * (which either prints out an img tag or sends a direct request
     * to AppMeasurement Collection Servers), using an associative array
     ...
     * Track the current state of the AppMeasurement instance
     * (which either prints out an img tag or sends a direct request
     * to Adobe Collection Servers), corresponding to a specific
    ...
    /**
     * Clear the majority of variables currently set on the AppMeasurement instance.
     */
    ...
    /**
     * Allow the AppMeasurement instance to manage visitor identification
     * instead of Adobe Collection Servers.
     *
    ...
    private function log($title, $message)
    {
        ...
        if ($s->debugTracking) {
            $debugContents = "AppMeasurement Debug [$title]: $message\n";
    ...
    private function getDefaultPageURL()
    {
        if (isset($_SERVER['REQUEST_URI'])) {
            return ($this->ssl ? 'https://' : 'http://') . $_SERVER['SERVER_NAME'] . $_SERVER['REQUEST_URI'];
        } else {
            return '';
        }
    }
